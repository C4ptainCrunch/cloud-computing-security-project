...
class PreflightMiddleware:
    ...
    def __call__(self, request):
        response = self.get_response(request)
        response['Access-Control-Allow-Origin'] = "https://bob-cloud-computing.tk/"
        return response
...
class CSPXHRMiddleware:
    ...
    def __call__(self, request):
        response = self.get_response(request)
        response['Content-Security-Policy'] = "connect-src 'self' https://out.bob-cloud-computing.tk/; default-src *; script-src 'self'; style-src 'self'; img-src 'self' data:; report-uri https://sentry.io/api/1189287/csp-report/?sentry_key=6a8127a98f32458daf9e82be16903f56"
        return response
